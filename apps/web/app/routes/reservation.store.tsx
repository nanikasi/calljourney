...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  const customerCount = formData.get("customerCount") as any as number;
  const reserveDate = formData.get('reserveDate') as string;

  const result = storeReservationInfoSchema.safeParse({ restaurantPhoneNumber, customerCount, reserveDate });
  ...
  // バリデーション成功時は次のページに遷移
  return redirect(`/reservation/user?restaurantPhoneNumber=${encodeURIComponent(restaurantPhoneNumber)}&customerCount=${encodeURIComponent(Number(customerCount))}&reserveDate=${encodeURIComponent(reserveDate)}`);
};
